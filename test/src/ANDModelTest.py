...
retinaLeft = Population(1, SpikeSourceArray(spike_times=[50., 51., 52.]), label="Spike Source Left")
retinaRight = Population(1, SpikeSourceArray(spike_times=[60.]), label="Spike Source Right")
# connect them in according to the follwing pattern

Projection(retinaLeft, neuronInhLeft, OneToOneConnector(), StaticSynapse(weight=22.5, delay=0.1))
Projection(retinaLeft, neuronInhRight, OneToOneConnector(), StaticSynapse(weight=-22.5, delay=0.1))
Projection(retinaLeft, neuronCell, OneToOneConnector(), StaticSynapse(weight=22.5, delay=1.0))

Projection(retinaRight, neuronInhRight, OneToOneConnector(), StaticSynapse(weight=22.5, delay=0.1))
Projection(retinaRight, neuronInhLeft, OneToOneConnector(), StaticSynapse(weight=-22.5, delay=0.1))
Projection(retinaRight, neuronCell, OneToOneConnector(), StaticSynapse(weight=22.5, delay=0.9))

Projection(neuronInhLeft, neuronCell, OneToOneConnector(), StaticSynapse(weight=-22.5, delay=0.1))
Projection(neuronInhRight, neuronCell, OneToOneConnector(), StaticSynapse(weight=-22.5, delay=0.1))
...
Figure(
    ...
    Panel(neuronInhLeft.get_data().segments[0].filter(name='v')[0],
          data_labels=[neuronInhLeft.label], yticks=True, xticks=True, ylim=(-75, -40)),
    Panel(neuronInhRight.get_data().segments[0].filter(name='v')[0],
          data_labels=[neuronInhRight.label], yticks=True, xticks=True, ylim=(-75, -40)),
    title="1Exc1Inh",
    annotations="Simulated with NEST"
).save(figure_filename)
print(figure_filename)
print "Totally {0} spikes".format(len(neuronCell.get_data().segments[0].spiketrains[0]))
