...
dimensionRetinaY = 30       # Defines the dimension of the y-axis of the input spikes from the retina sensor
minDisparity = 0            # Defines the minimum detectable disparity
maxDisparity = 10            # Defines the maximum detectable disparity
...
experimentName = "artificial_events"
retLeftSpikes = load(open('../src/realInput/retinaLeft_{0}x{1}_{2}.p'.format(dimensionRetinaX, dimensionRetinaY, experimentName), 'rb'))
retRightSpikes = load(open('../src/realInput/retinaRight_{0}x{1}_{2}.p'.format(dimensionRetinaX, dimensionRetinaY, experimentName), 'rb'))

# from cPickle import load
# retLeftSpikes = load(open('../src/realInput/retinaLeft_50_persAway.p', 'rb'))
# retRightSpikes = load(open('../src/realInput/retinaRight_50_persAway.p', 'rb'))
